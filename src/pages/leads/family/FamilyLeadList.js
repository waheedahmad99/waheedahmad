...
import { updateContactList, updateFamilyLeadList } from '../../../store/action_calls';
import {connect,useDispatch} from 'react-redux'
import {cloneDeep} from 'lodash'
import ImportDragAndDrop from '../../contact/vendor/ImportDragAndDrop'
import { getFamilyLeads } from '../../../store/action_calls';
...
const App = props => {
    ...
    // useEffect(() => {
    //     setRowData(cloneDeep(Object.values(props.familyLeadList)))
    //     },
    //     [props.familyLeadList]);


        const endTrustIndex=useRef(0)
        useEffect(() => {
            if(!gridRef) return
              const allFamilyLeadList=cloneDeep(Object.values(props.familyLeadList))
              if(allFamilyLeadList.length==0) return
               if(rowData?.length==0) {
                   setRowData(allFamilyLeadList)
                   endTrustIndex.current=allFamilyLeadList.length
            }
               else if(endTrustIndex.current!=allFamilyLeadList.length) {
                   gridRef.current?.api?.addItems(allFamilyLeadList.slice(endTrustIndex.current,allFamilyLeadList.length))
                   endTrustIndex.current=allFamilyLeadList.length
               }
            },[props.familyLeadList,gridRef]);
    //this toggle above is how it knows to actually reload when toggle is called

    const [reloadFamilyLeads,setReloadFamilyLead]=useState(null)
   
    useEffect(()=>{
     if(reloadFamilyLeads=='reload'){
         props.getFamilyLeads()
         setReloadFamilyLead(null)
     }
    ...
    const handleUpdate = async(body) => {
      ...
      if(data){
          ...
          dispatch(updateFamilyLeadList({...body.data,...obj}))
    ...
    const handleGridReady = params => {
        ...
        //quickfilter!
        const [quickfilter,setQuickfilter]=useLocalStorage('familylead_list_filter_text','')
        useEffect(()=>{
            if(gridApi){
        ...
        function onFilterTextBoxChanged() {
            ...
            const [sortConfig,setSortConfi]=useLocalStorage('familylead_list_order',[])
            const [filterConfig,setFilterConfi]=useLocalStorage('familylead_list_filter',[])
            ...
            <Card.Body>
                <div className="ag-theme-alpine" style={{ height: "100%" }}>
                    ...
                    {/* <Button className="shadow-1 theme-bg border border-0"
                        ...
                        Create </Button> */}
                        <CreateContact name='Family Lead' />
                    ...
                    <input type="text" className="form-control " id="filter-text-box"
                        ...
                        <ImportDragAndDrop isFamilyLead={true} reloadPage={()=>setReloadFamilyLead('reload')}/>
                    <AgGridReact
...
const mapStateToProps = state => ({
  ...
  export default connect(mapStateToProps, {getFamilyLeads})(App)
