...
import { getInvestors } from '../../store/action_calls';
...
const App = props => {
    ...
    const [reloadInvestors,setReloadInvestors]=useState(null)
   
    useEffect(()=>{
     if(reloadInvestors=='reload'){
         props.getInvestors()
         setReloadInvestors(null)
     }
    ...
    useEffect(() => {
        if(!gridRef) return
          const allInvestorList=cloneDeep(Object.values(props.investorList))
          if(allInvestorList.length==0) return
           if(rowData?.length==0) {
               setRowData(allInvestorList)
               endTrustIndex.current=allInvestorList.length
        }
           else if(endTrustIndex.current!=allInvestorList.length) {
               gridRef.current?.api?.addItems(allInvestorList.slice(endTrustIndex.current,allInvestorList.length))
               endTrustIndex.current=allInvestorList.length
           }
        },[props.investorList,gridRef]);

    // useEffect(() => {
    //     setRowData(cloneDeep(Object.values(props.investorList)))
    //     },
    //     [props.investorList]);
    ...
    const handleUpdate = async(body) => {
        console.log("Update", body.data, "Field:", body.colDef.field, "New Value:", body.newValue)
       ...
       if(data){
        ...
        dispatch(updateInvestorList({...body.data,...obj}))
       }
    ...
    const handleGridReady = params => {
        ...
        //quickfilter!
        const [quickfilter,setQuickfilter]=useLocalStorage('investor_list_filter_text','')
        useEffect(()=>{
            if(gridApi){
        ...
        function onFilterTextBoxChanged() {
            ...
            const [sortConfig,setSortConfi]=useLocalStorage('investor_list_order',[])
            const [filterConfig,setFilterConfi]=useLocalStorage('investor_list_filter',[])
            ...
            <Card.Body>
                <div className="ag-theme-alpine" style={{ height: "100%" }}>
                    ...
                    <input type="text" className="form-control " id="filter-text-box"
                        ...
                        <ImportDragAndDrop isInvestor={true}  reloadPage={()=>setReloadInvestors('reload')}/>
                    <AgGridReact
...
const mapStateToProps = state => ({
  ...
  export default connect(mapStateToProps, {getInvestors})(App)
