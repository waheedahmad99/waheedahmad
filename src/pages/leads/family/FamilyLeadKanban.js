...
import {FamilyLeads,Contacts,Lanes} from '../../../services/'
import { useLocalStorage } from '../../../utils/useLocalStorage';
import _ from 'lodash'
import httpService from '../../../services/httpService';
import { connect,useDispatch } from 'react-redux';
import {updateFamilyLeadList} from '../../../store/action_calls'

const TrustTaskBoard = (props) => {
    const [board, setBoard] = useState({lanes: []})
    const [trustsData,setTrustsData]=useState([])
    const [groupBy,setGroupBy]=useLocalStorage('groupByFamilyLead','lead_stage')
    ...
    useEffect(()=>{
        ...
        const getStage=async()=>{
            const data=await FamilyLeads.getSetups(source)
            data&&setSetups(data)
        }
        getStage()
    ...
    useEffect(()=>{
        ...
        const getReqairStage=async()=>{
            const data=await  FamilyLeads.getRepairStages(source)
    ...
    useEffect(() => {
       ...
       const getTrust=async()=>{
        ...
        if(Object.keys(props.familyLeadList).length!=0){
        const data=Object.values(props.familyLeadList)
   ...
   [groupBy,setUps,repairStage])
    ...
    const handleDragEnd = async( cardId, sourceLaneId, targetLaneId, position, cardDetails) => {
        ...
        const data=await FamilyLeads.updateFamilyLead(id,obj)
        data&&dispatch(updateFamilyLeadList(data))
