...
import { getFamilies } from '../../store/action_calls';
...
const App = props => {
   ...
   const [reloadFamilies,setReloadFamilies]=useState(null)
   
   useEffect(()=>{
    if(reloadFamilies=='reload'){
        props.getFamilies()
        setReloadFamilies(null)
    }
   ...
   useEffect(() => {
       if(!gridRef) return
         const allFamilyList=cloneDeep(Object.values(props.familyList))
         if(allFamilyList.length==0) return
          if(rowData?.length==0) {
              setRowData(allFamilyList)
              endTrustIndex.current=allFamilyList.length
       }
          else if(endTrustIndex.current!=allFamilyList.length) {
              gridRef.current?.api?.addItems(allFamilyList.slice(endTrustIndex.current,allFamilyList.length))
              endTrustIndex.current=allFamilyList.length
          }
       },[props.familyList,gridRef]);

    // useEffect(() => {
    //     setRowData(cloneDeep(Object.values(props.familyList)))
    //     },
    //     [props.familyList]);
    ...
    const handleUpdate = async(body) => {
       ...
       if(data){
           ...
           dispatch(updateFamilyList({...body.data,...obj}))
    ...
    const handleGridReady = params => {
        ...
        //quickfilter!
        const [quickfilter,setQuickfilter]=useLocalStorage('family_list_filter_text','')
        useEffect(()=>{
            if(gridApi){
        ...
        function onFilterTextBoxChanged() {
            ...
            const [sortConfig,setSortConfi]=useLocalStorage('family_list_order',[])
            const [filterConfig,setFilterConfi]=useLocalStorage('family_list_filter',[])
            ...
            <Card.Body>
                <div className="ag-theme-alpine" style={{ height: "100%" }}>
                    ...
                    <input type="text" className="form-control " id="filter-text-box"
                        ...
                        <ImportDragAndDrop isFamily={true} reloadPage={()=>setReloadFamilies('reload')}/>
                    <AgGridReact
