      ...
      >
          ...
          onCloseClick={()=>setIsOpen(null)}
            ...
            <div style={{ opacity: 0.75, padding: `8px` }}>
                    <div style={{ fontColor: `#08233B` }}  onClick={()=>history.push(`/investor/${address.id}`)}>
  ...
  useEffect(() => {
   ...
   const getAddresses=async()=>{
       const addressesData=Object.values(props.addressList)
        const investorsData=Object.values(props.investorList)
         const addressForInvestor=investorsData.reduce((results,trust)=>{
        ...
        setAddresses(addressForInvestor)
   }
   getAddresses()
